...
|---|---|
| 2026-08-18 (round 44) | **Fix Global Estimator 1.0 KB Bug & Community Compression Algorithm Integration**: **Fix Global Estimator Math**: Mengubah `createEstimatorCard` untuk menerima `(origBytes, estBytes)` langsung sehingga telemetry card dan staged file badge sinkron secara presisi (misal PDF 100MB pada 65% quality menampilkan ~52MB / -48%, bukan 1.0 KB); **Crosscheck & Integrasi Algoritma Komunitas**: (1) **PDF Engine**: Mengintegrasikan hybrid `pdf-lib` + `pdfjs-dist` high-DPI rendering dengan JPEG quantization & object stream optimization; (2) **Image Engine**: Mengadopsi algoritma `browser-image-compression` binary search multi-pass (clamp 4096px texture, WebP auto-transparency, smoothing filter, ceiling rule); (3) **Audio Engine**: `OfflineAudioContext` sample rate downsampling & mono mixing; (4) **Video/GIF Engine**: `MediaRecorder` dengan `videoBitsPerSecond` bitrate budgeting; Unit test suite 40/40 lulus, lint/typecheck 0 error, build production berhasil. |
